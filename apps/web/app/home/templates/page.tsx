...
import { TemplateGuide, TEMPLATE_GUIDES } from "@/components/workflows/TemplateGuide";
const TIMING = {
  headerDuration: 0.2,
  cardStagger: 0.06,
  cardDelayStart: 0.15,
...
export default function TemplatesPage() {
  ...
  const [showingGuideFor, setShowingGuideFor] = useState<string | null>(null);
  ...
  const handleUseTemplate = async (templateId: string) => {
    ...
    // Strip prefixes like 'b2b-', 'whatsapp-' from the internal ID to match the TEMPLATE_GUIDES key
    const guideKeyMatch = template.id.match(/^.*?-(.*)$/);
    const guideKey = guideKeyMatch?.[1] ?? template.id; // Usually e.g. "mpesa-payment", "sms-notification"

    // If there's a guide, pop the modal first. Otherwise deploy immediately.
    if (TEMPLATE_GUIDES[guideKey]) {
      setShowingGuideFor(guideKey);
    } else {
  ...
  return (<div className="flex-1 flex flex-col">
    ...
    {showingGuideFor && TEMPLATE_GUIDES[showingGuideFor] && (
      <TemplateGuide
        open={true}
        onClose={() => setShowingGuideFor(null)}
        templateName={WORKFLOW_TEMPLATES.find((t) => (t.id.match(/^.*?-(.*)$/)?.[1] || t.id) === showingGuideFor)?.name || "Template Guide"}
        description={TEMPLATE_GUIDES[showingGuideFor].description}
        steps={TEMPLATE_GUIDES[showingGuideFor].steps}
        onConfirm={() => {
          const template = WORKFLOW_TEMPLATES.find((t) => (t.id.match(/^.*?-(.*)$/)?.[1] || t.id) === showingGuideFor);
          if (template) createWorkflowFromTemplate(template);
        }}
      />
    )}
  </div>);
